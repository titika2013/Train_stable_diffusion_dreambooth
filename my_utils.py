...
def create_images_folder(main_dir, instance_dir, images_folder, remove_existing_instance_images=True):
    os.chdir(main_dir)
    ...
    if images__folder != "":
        for _ in tqdm.tqdm(os.listdir(images_folder), bar_format='  |{bar:15}| {n_fmt}/{total_fmt} Uploaded'):
...
def save_model_new_place(session_name, prev_model_out_dir, path_to_dest="", main_dir=os.getcwd()):
    if not path_to_dest:
        path_to_dest = main_dir + "/" + session_name
    ...
    if os.path.exists(prev_model_out_dir + '/unet/diffusion_pytorch_model.bin'):
        print("Almost done ...")
        if not os.path.exists(path_to_dest):
            os.mkdir(path_to_dest)
        if not need_to_copy:
            shutil.move(prev_model_out_dir, path_to_dest)
        else:
            copy_tree(prev_model_out_dir, path_to_dest)
        # copy_tree(IMAGES_FOLDER, PATH_TO_DEST + "/" + IMAGES_FOLDER)
        # with open(PATH_TO_DEST + "/Token_name.txt", 'w') as f:
        #     f.write(KEY_NAME)
    print('[1;31m Save model in new place Done')
...
def training_function(text_encoder, vae, unet, args, tokenizer):
